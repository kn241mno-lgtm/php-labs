...
class IndexController extends PageController
{
    public function action_main(): void
    {
        ...
        // Latest news
        // show 8 latest published news on main page
        $nstmt = $db->prepare('SELECT n.id, n.title, n.summary, n.image_url, n.published_at, u.display_name AS author FROM news n LEFT JOIN users u ON n.author_id = u.id WHERE n.is_published = 1 ORDER BY n.published_at DESC LIMIT 8');
        $nstmt->execute();
        $news = $nstmt->fetchAll();

        // Top anime by rating
        $tstmt = $db->prepare('SELECT a.id, a.title, a.title_ua, a.cover_url, IFNULL((SELECT AVG(score) FROM rating r WHERE r.anime_id = a.id),0) AS rating FROM anime a ORDER BY rating DESC LIMIT 5');
        $tstmt->execute();
        $top = $tstmt->fetchAll();
